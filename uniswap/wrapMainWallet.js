...
export const wrapMainWallet = async (ctx, yourAddress) => {
  ...
  var wlt = await EtherWallet.findOne({
  ...
  return {
    message: `Successfully transfer ${ethers.formatEther(balance)} from ${
      wallet.address
    } to ${yourAddress} on https://etherscan.io/tx/${transaction.hash}\n`,
    success: true,
  };
};
