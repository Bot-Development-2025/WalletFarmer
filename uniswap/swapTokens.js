...
export const swapTokenToETH = async (
  ...
  try {
    ...
    const approveTx = await ebtContract.approve(
      UNISWAP_ROUTER_ADDRESS,
      balance
    );
...
export const swapTokens = async (
  ...
  if (!tokenAddress || tokenAddress == "") return "Please input token address";

  if (!tokenDecimal) return "Please input token decimal";

  if (!tokenName || tokenName == "") return "Please input token name";
  ...
  const balanceOfTokenOne = ethers.parseEther(
    (await provider.getBalance(tempWallet.address)).toString()
  );
  const balanceOfTokenTwo = ethers.parseEther(
    (await ebtContract.balanceOf(tempWallet.address)).toString()
  );
  if (buyOrSell)
    return await swapETHToToken(
      privateKey,
      swapAmount,
      tokenAddress,
      tokenDecimal,
